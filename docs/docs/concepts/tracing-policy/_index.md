...
[hook points page](/docs/docs/concepts/tracing-policy/hooks) and the
[selectors page](/docs/docs/concepts/tracing-policy/selectors).

For the complete custom resource definition (CRD) refer to
[Tracing Policy API](/docs/docs/reference/tracing-policy)
documentation.
...
- You can use the `--tracing-policy` and `--tracing-policy-dir` flags to statically add policies at
  startup time, see more in the [daemon configuration page](/docs/docs/reference/daemon-configuration#configure-tracing-policies-location).
